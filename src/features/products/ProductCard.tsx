...
const ProductCard = memo(function ProductCard({
  ...
  const stockStatus = useMemo(() => {
    ...
    if (stock <= 5)
      return { label: `Only ${stock} left`, color: "warning" as const };
    return { label: "In stock", color: "success" as const };
  }, [stock]);

  console.log(
    `ProductCard ${id} render: discountedPrice=${discountedPrice}, stockStatus=${stockStatus.label} , color=${color}`,
  );
        ...
        <Box sx={{ position: "relative" }}>
          ...
          <Box
            sx={{
              ...
              top: 8,
              right: 8,
              bgcolor: "rgba(246, 241, 241, 68)",
              borderRadius: "100%",
            }}
          >
            <WishlistButton productId={id} size="medium" />
          </Box>
        </Box>
